<div class="py-24 bg-black text-white" id="location">
    ...
    <div class="container mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-16 items-start">
        ...
        <div>
            ...
            {{-- Map menggunakan zoom-in agar ada efek pop-up visual --}}
        ...
        <div>
            ...
            {{-- Form muncul terakhir (delay 400ms) --}}
            <form action="{{ route('comment.store') }}" method="POST" class="mt-8"
                ...
                <div class="flex rounded-lg overflow-hidden border border-neutral-700 
                            focus-within:ring-2 focus-within:ring-white transition-all duration-300">
                    <input 
                        type="text" 
                        name="comment"
                ...
                @if(session('success'))
                    <p class="text-green-500 text-sm mt-2 animate-bounce">{{ session('success') }}</p>
                @endif
            </form>
        </div>
